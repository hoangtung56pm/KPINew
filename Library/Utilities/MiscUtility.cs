...
namespace VmgPortal.Library.Utilities
{
	public class MiscUtility
	{
        ...
        public const string MSG_UPDATE_CONFIRM = "return confirm('Bạn đã chắc chắn cập nhật ?');";
        public const string MSG_EXISTED_CONFIRM = "Nội dung này đã tồn tại! Bạn phải nhập lại";
		public const string MSG_UPDATE_SUCCESS = "<b>Bạn đã cập nhật thành công !</b>";
        ...
        public static string SuccessAlert(string item1, string item2)
        {
            return "<b>Bạn đã thêm mới thành công " + item1 + " <b class=\"redtext\">" + item2 + "</b></b>";
        }

        public static string SuccessAlert(string item1, string item2, string item3)
        {
            return "<b>Bạn đã cập nhật thành công " + item1 + " <b class=\"redtext\">" + item2 + "</b></b> vào chuyên mục <b class=\"redtext\">" + item3 + "</b> trên Wap";
        }

        public static string DeleteAlert(string item1, string item2)
        {
            return "<b>Bạn đã xóa thành công " + item1 + " <b class=\"redtext\">" + item2 + "</b></b>";
        }
		...
		public static string UpdateQueryStringItem(HttpRequest httpRequest, string[] queryStringKeys, string[] newQueryStringValues)
		{
			...
			for (int i = 0; i < queryStringKeys.GetLength(0); i ++)
			{
				...
				if (httpRequest.QueryString[queryStringKey] != null)
				{
					string OrignalSet = String.Format("{0}={1}", queryStringKey, httpRequest.QueryString[queryStringKey]);
					string NewSet = String.Format("{0}={1}", queryStringKey, newQueryStringValue);
					NewURL.Replace(OrignalSet.Replace(" ","+"), NewSet);
				}
				else if (httpRequest.QueryString.Count == 0)
				{
