...
{
    public partial class DanhGiaThang_LanhDao_KD : AuthenticatedControl
    {
        int idDotDanhGia = 0;
        DataTable dt;
        protected void Page_Load(object sender, EventArgs e)
        {
            ...
            if (!IsPostBack)
            {
                ...
                if (idDotDanhGia == 0)
                {
                    ...
                    Response.Redirect(AppEnv.AdminUrlParams("danhgiathang_ld_kd") + "&idDotDanhGia=" + idDotDanhActive);
                }
                else
                {
        ...
        protected override void OnPreRender(EventArgs e)
        {
            ...
            DataTable dtUsers = UserController.GetLanhDaoBP(txtKeyword.Text, dropSearchBy.SelectedValue, ConvertUtility.ToString(ViewState["Alphabet"]), idDotDanhGia, 0);
        ...
        protected void dtgUsers_RowDataBound(object sender, GridViewRowEventArgs e)
        {
            if (e.Row.RowType == DataControlRowType.DataRow)
            {
                ...
                if (ConvertUtility.ToInt32(curData["UserID"].ToString()) == 2)
                {
                    string url = "window.open('" + AppEnv.AdminUrlParams("dg_danhgiacoo") + "&idDotDanhGia=" + idDotDanhGia + "&idNhanVien=" + curData["UserID"].ToString() + "','_self'); return false;";
                    btnDanhGiaTruongPhong.OnClientClick = url;
                }
                else
                {
                    string url = "window.open('" + AppEnv.AdminUrlParams("dg_danhgiald_kd") + "&idDotDanhGia=" + idDotDanhGia + "&idNhanVien=" + curData["UserID"].ToString() + "','_self'); return false;";
                    btnDanhGiaTruongPhong.OnClientClick = url;
                }
            }
        }
